...
def get_application_name():
    if os.path.exists('.qs'):
        with open('.qs', 'r') as file:
            app_name = file.readline().strip()
            return app_name
    else:
...
def create_network():
    result = subprocess.run(["docker", "network", "inspect", "quickstack-platform-interconnect"], capture_output=True)
    
    if result.returncode != 0:
        print_warning("Container network doesn't exist yet...")
        result = subprocess.run(["docker", "network", "create", "-d", "bridge", "quickstack-platform-interconnect"], capture_output=True)
        if (result.returncode != 0):
...
def build_compose_stack(args):
    ...
    if result.returncode != 0:
        print_error(f"Failed to build application images! Try running with --debug for more details")
    print_success(f"Successfully built application images!") 
    
def start_compose_stack(args):
    print_info(f"Starting stack...")        
    result = subprocess.run(["docker", "compose", "up", "-d"], capture_output=not args.debug)        
    if result.returncode != 0:
        print_error(f"Failed to start stack!")
    print_success(f"Successfully started stack!") 
    if (args.attach):
        subprocess.run(["docker", "compose", "logs", "--follow", get_application_name()], capture_output=False) 

def restart_compose_stack(args):
    print_info(f"Restarting stack...")        
    result = subprocess.run(["docker", "compose", "restart"], capture_output=not args.debug)        
    if result.returncode != 0:
...
def stop_compose_stack(args):
    ...
    result = subprocess.run(["docker", "compose", "stop"], capture_output=not args.debug)        
    if result.returncode != 0:
...
def destroy_compose_stack(args):
    ...
    result = subprocess.run(["docker", "compose", "down"], capture_output=not args.debug)        
    if result.returncode != 0:
        print_error(f"Failed to destroy stack!")
    print_success(f"Successfully destroyed stack!") 
        
def ssh_application(args):
    result = subprocess.run(["docker", "compose", "exec", get_application_name(), "/bin/bash"], capture_output=False)    
...
def logs_from_application(args):
    result = subprocess.run(["docker", "compose", "logs", "--follow", get_application_name()], capture_output=False) 
    if result.returncode != 0:
...
def get_parser():
    ...
    build_command = subparsers.add_parser('build', help="Builds the application image")
    ...
    deploy_command = subparsers.add_parser('deploy', help="Builds the application image and then immediately starts the stack")
    ...
    ssh_command = subparsers.add_parser('ssh', help="SSH into an application container in the Docker stack")
    ...
    restart_command = subparsers.add_parser('restart', help="Restarts the entire stack")
    ...
    destroy_command = subparsers.add_parser('destroy', help="Destroys the stack")
